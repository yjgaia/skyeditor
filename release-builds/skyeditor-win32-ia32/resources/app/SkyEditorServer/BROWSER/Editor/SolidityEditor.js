SkyEditorServer.SolidityEditor = CLASS((cls) => {
	
	let getName = cls.getName = () => {
		return 'SkyEditor.SolidityEditor';
	};
	
	let getIcon = cls.getIcon = () => {
		return IMG({
			src : SkyEditor.R('icon/solidity.png')
		});
	};
			...
			if (global.web3 !== undefined && ftpInfo === undefined) {
				...
				self.on('save', () => {
					...
					if (code !== '') {
						...
						loadImportFiles(code, folderPath, '', () => {
							...
							// 저장되어 있던 계약 제거
							SkyEditorServer.EthereumContractModel.removeContractInfos(path, () => {
								
								SkyEditorServer.EthereumContractModel.compileSolidityCode({
									fileName : fileName,
									code : code,
									...
									success : (contractInfos) => {
										...
										// 컴파일 결과 저장
										SkyEditorServer.EthereumContractModel.saveContractInfos({
											path : path,
				...
				self.on('active', () => {
					...
					if (deployButton === undefined) {
						...
						// 도구 메뉴 추가
						SkyEditor.IDE.addToolbarButton(deployButton = SkyDesktop.ToolbarButton({
							icon : IMG({
								src : SkyEditor.R('icon/deploy.png')
							}),
							title : '계약 배포하기',
							on : {
								tap : (e) => {
									
									SkyEditorServer.EthereumContractModel.getContractInfos(path, (contractInfos) => {
												...
												EACH(contractInfos, (contractInfos) => {
													EACH(contractInfos, (contractInfo, name) => {
														...
														menu.append(SkyDesktop.ContextMenuItem({
															title : name,
															icon : IMG({
																src : SkyEditor.R('icon/contract.png')
															}),
															on : {
																tap : () => {
																			...
																			contractArgs.push((error, contract) => {
																				if (error !== TO_DELETE) {
																					...
																					if (contract.address !== undefined) {
																						...
																						SkyEditorServer.EthereumContractModel.create({
																							path : path,
																							name : name,
						...
						SkyEditor.IDE.addToolbarButton(testButton = SkyDesktop.ToolbarButton({
							icon : IMG({
								src : SkyEditor.R('icon/test.png')
							}),
							title : '계약 테스트',
							on : {
								tap : (e) => {
									
									SkyEditorServer.EthereumContractModel.find({
										filter : {
											path : path
										},
										count : 10
										...
										menu.append(SkyDesktop.ContextMenuItem({
											title : '주소로 계약 추가',
											icon : IMG({
												src : SkyEditor.R('icon/contract-add.png')
											}),
											on : {
												tap : (e) => {
						...
						let networkTitle;
						SkyEditor.IDE.addToolbarButton(networkButton = SkyDesktop.ToolbarButton({
							style : {
								cursor : 'default'
							},
							icon : IMG({
								src : SkyEditor.R('icon/network.png')
							}),
				...
				let removeToolbarButtons = () => {
					...
					if (deployButton !== undefined) {
						...
						// 도구 메뉴 제거
						SkyEditor.IDE.removeToolbarButton(deployButton);
						SkyEditor.IDE.removeToolbarButton(testButton);
						SkyEditor.IDE.removeToolbarButton(networkButton);
