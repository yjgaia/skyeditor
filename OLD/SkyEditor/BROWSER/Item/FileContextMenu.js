SkyEditor.FileContextMenu = CLASS({
	...
	init : (inner, self, params) => {
		...
		let selectedFileItems = SkyEditor.IDE.getSelectedFileItems();
							...
							RUN((f) => {
								...
								SkyEditor.IDE.checkExists({
									...
									if (exists === true) {
										...
										SkyEditor.IDE.saveTab(SkyEditor.IDE.openEditor(SkyEditor.IDE.getEditor(fileName.substring(fileName.lastIndexOf('.') + 1).toLowerCase())({
											ftpInfo : ftpInfo,
											title : fileName,
											path : folderPath + '/' + fileName
										})));
		...
		self.append(SkyDesktop.ContextMenuItem({
			title : '붙여넣기',
			icon : IMG({
				src : SkyEditor.R('icon/paste.png')
			}),
			on : {
				tap : () => {
		...
		if (path !== undefined && selectedFileItems.length === 1) {
			...
			self.append(SkyDesktop.ContextMenuItem({
				title : '이름 변경',
				icon : IMG({
					src : SkyEditor.R('icon/rename.png')
				}),
				on : {
					tap : () => {
			...
			self.append(SkyDesktop.ContextMenuItem({
				title : '파일 정보',
				icon : IMG({
					src : SkyEditor.R('icon/info.png')
				}),
				on : {
					tap : () => {
						
						SkyEditor.IDE.getInfo({
							ftpInfo : ftpInfo,
							path : path
						}, (info) => {
			...
			if (selectedFileItems[0].checkIsInstanceOf(SkyEditor.File) === true) {
				
				self.append(SkyDesktop.ContextMenuItem({
					title : '로컬 저장 기록 보기',
					icon : IMG({
						src : SkyEditor.R('icon/history.png')
					}),
					on : {
						tap : () => {
							...
							SkyEditor.IDE.addTab(tab = SkyDesktop.Tab({
								style : {
									position : 'relative'
								},
								size : 30,
							...
							SkyEditor.IDE.load({
								...
								let history = SkyEditor.IDE.getLocalHistory({
									ftpInfo : ftpInfo,
									path : path
								});
								...
								REVERSE_EACH(history, (info) => {
									...
									if (info.content !== content) {
										...
										historyList.append(SkyEditor.HistoryItem({
											title : title,
											on : {
												doubletap : () => {
													
													SkyEditor.IDE.openEditor(SkyEditor.CompareEditor({
		...
		if (ftpInfo === undefined && selectedFileItems.length > 0) {
			...
			self.append(SkyDesktop.ContextMenuItem({
				title : '검색',
				icon : IMG({
					src : SkyEditor.R('icon/search.png')
				}),
				on : {
					tap : () => {
						
						SkyEditor.IDE.search();
		...
		if (selectedFileItems.length >= 2) {
			...
			EACH(selectedFileItems, (item) => {
				if (item.checkIsInstanceOf(SkyEditor.File) === true) {
					if (item1 === undefined) {
						item1 = item;
					} else {
						item2 = item;
			...
			if (item2 !== undefined) {
				...
				self.append(SkyDesktop.ContextMenuItem({
					title : '두 파일 비교',
					icon : IMG({
						src : SkyEditor.R('icon/compare.png')
					}),
					on : {
						tap : () => {
		...
		if (folderPath.substring(0, folderPath.lastIndexOf('/')) === SkyEditor.IDE.getWorkspacePath()) {
			...
			self.append(SkyDesktop.ContextMenuItem({
				...
				on : {
					tap : () => {
						...
						SkyEditor.IDE.openEditor(SkyEditor.GitEditor({
							folderPath : folderPath
						}));
