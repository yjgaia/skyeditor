SkyEditorServer.Home = CLASS({
	...
	init : (inner, self) => {
		...
		let editorStore = SkyEditorServer.STORE('editorStore');
		let folderOpenedStore = SkyEditorServer.STORE('folderOpenedStore');
		
		let apiRoom = SkyEditorServer.ROOM('API');
		...
		() => {
			return () => {
				...
				SkyEditor.IDE.init({
					...
					// 홈 화면 띄우기
					showHome : () => {
						SkyEditor.IDE.openEditor(SkyEditor.HomeTab(BROWSER_CONFIG.SkyEditor.homepage + '?version=' + CONFIG.version));
					},
					...
					save : (path, content, buffer, errorHandler, callback, isFindAndReplace) => {
						...
						NEXT([(next) => {
							...
							if (path === undefined) {
								...
								// 저장 위치 선택 창 띄우기
								SkyEditor.CreateSelectFolderPopup(folderOpenedStore, next);
							}
					...
					ftpNew : (ftpInfo, errorHandler, existedHandler, callback) => {
						...
						SkyEditorServer.FTPModel.checkExists(ftpInfo.username + '@' + ftpInfo.host, (exists) => {
							...
							if (exists === true) {
								...
								SkyEditorServer.FTPModel.create({
					...
					ftpDestroy : (ftpInfo, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.remove(ftpInfo.username + '@' + ftpInfo.host, {
					...
					ftpConnect : (ftpInfo, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.connect(ftpInfo, {
							error : errorHandler,
							success : callback
						});
					},
					...
					ftpLoadFiles : (ftpInfo, path, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.loadFiles({
							ftpInfo : ftpInfo,
							path : path
						}, {
							error : errorHandler,
					...
					ftpLoad : (ftpInfo, path, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.loadFile({
							ftpInfo : ftpInfo,
							path : path
						}, {
							error : errorHandler,
					...
					ftpGetInfo : (ftpInfo, path, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.getFileInfo({
							ftpInfo : ftpInfo,
							path : path
						}, {
							error : errorHandler,
					...
					ftpCheckExists : (ftpInfo, path, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.checkFileExists({
							ftpInfo : ftpInfo,
							path : path
						}, {
							error : errorHandler,
					...
					ftpSave : (ftpInfo, path, content, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.saveFile({
							ftpInfo : ftpInfo,
							path : path,
							content : content
						}, {
					...
					ftpCreateFolder : (ftpInfo, path, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.createFolder({
							ftpInfo : ftpInfo,
							path : path
						}, {
							error : errorHandler,
					...
					ftpMove : (fromFTPInfo, toFTPInfo, from, to, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.moveFile({
					...
					ftpClone : (fromFTPInfo, toFTPInfo, from, to, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.cloneFile({
							fromFTPInfo : fromFTPInfo,
							toFTPInfo : toFTPInfo,
							from : from,
							to : to
					...
					ftpRemove : (ftpInfo, path, errorHandler, callback) => {
						...
						SkyEditorServer.FTPModel.removeFile({
							ftpInfo : ftpInfo,
							path : path
						}, {
							error : errorHandler,
									...
									RUN((f) => {
										...
										() => {
											return () => {
												...
												SkyEditorServer.FTPModel.checkIsFolder({
													...
													success : (isFolder) => {
														...
														if (isFolder === true) {
															...
															if (clipboardPathInfo.ftpInfo === ftpInfo) {
																...
																SkyEditorServer.FTPModel.copyFolder({
																	ftpInfo : ftpInfo,
																	from : path,
														...
														// 파일 복사
															...
															if (clipboardPathInfo.ftpInfo === ftpInfo) {
																SkyEditorServer.FTPModel.copyFile({
																	ftpInfo : ftpInfo,
																	from : path,
																	to : folderPath + '/' + fileName
																}, {
				...
				SkyEditor.IDE.addToolbarButton(SkyDesktop.ToolbarButton({
					icon : IMG({
						src : SkyEditor.R('icon/logout.png')
					}),
					title : '로그아웃',
					on : {
						tap : () => {
				...
				RUN(() => {
					...
					let createFolderItem = (path, folderName) => {
						...
						// 폴더 관리 룸
						let room = SkyEditorServer.ROOM('Folder' + path);
					...
					let createFileItem = (path, fileName) => {
						...
						// 파일 관리 룸
						let room = SkyEditorServer.ROOM('File' + path);
						
						let file = SkyEditor.File({
							path : path,
							title : fileName
						});
						...
						room.on('remove', () => {
							...
							let opendEditor = SkyEditor.IDE.getOpenedEditor(path);
					...
					loadFolderFiles('', SkyEditor.IDE);
					
					// 워크스페이스 관리 룸
					let room = SkyEditorServer.ROOM('Folder/');
					
					SkyEditor.IDE.setWorkspacePath('');
					
					// 워크스페이스 관리 룸
					let workspaceRoom = SkyEditorServer.ROOM('Folder');
				...
				// FTP 정보 로드
				SkyEditorServer.FTPModel.find(EACH((ftpInfo) => {
