SkyEditor.IDE = OBJECT({
	...
	init : (inner, self) => {
		...
		let editorMap = {};
		let editorSettingStore = SkyEditor.STORE('editorSettingStore');
		let editorOpenedStore = SkyEditor.STORE('editorOpenedStore');
		...
		let getEditor = self.getEditor = (extname) => {
			...
			let SelectedEditor = SkyEditor.TextEditor;
			...
			c : TD({
				...
				c : toolbar = SkyDesktop.Toolbar({
					buttons : [SkyDesktop.ToolbarButton({
						...
						on : {
							tap : () => {
								openEditor(SkyEditor.TextEditor({
									title : '제목 없음'
								}));
							}
						}
						...
						on : {
							tap : () => {
								...
								if (activeTab.checkIsInstanceOf(SkyEditor.Editor) === true) {
									saveTab(activeTab);
								}
							}
						}
							...
							tap : () => {
								...
								SkyDesktop.Confirm({
									...
									if (validResult.checkHasError() === true) {
										...
										let folderPath = SkyEditor.IDE.getWorkspacePath() + '/' + data.folderName;
		...
		let openEditor = self.openEditor = (tab) => {
			...
			if (
			tab.checkIsInstanceOf(SkyEditor.Editor) === true &&
			tab.checkIsInstanceOf(SkyEditor.CompareEditor) !== true &&
		...
		let loadAndOpenEditor = (path, scrollTop, findText) => {
			...
			EACH(editorGroup.getAllTabs(), (tab, i) => {
				if (tab.checkIsInstanceOf(SkyEditor.Editor) === true && tab.getFTPInfo() === undefined && tab.getPath() === path) {
		...
		let addFTPItem = self.addFTPItem = (ftpInfo) => {
			...
			ftpFileTree.addItem({
				...
				item : item = SkyEditor.FTPItem(ftpInfo)
			});
		...
		let saveTab = self.saveTab = (tab) => {
			...
			if (tab.checkIsInstanceOf(SkyEditor.GitEditor) === true) {
				// ignore.
			}
			
			else if (tab.checkIsInstanceOf(SkyEditor.CompareEditor) === true) {
				...
				return () => {
					...
					if (fromFTPInfo !== undefined || toFTPInfo !== undefined) {
						...
						ftpMoveHandler(fromFTPInfo, toFTPInfo, from, to, () => {
							...
							if (fromParentItem.getItem(from).checkIsInstanceOf(SkyEditor.Folder) === true) {
		...
		let getOpenedEditor = self.getOpenedEditor = (path) => {
			...
			EACH(editorGroup.getAllTabs(), (tab) => {
				if (tab.checkIsInstanceOf(SkyEditor.Editor) === true && tab.getPath() === path) {
					editor = tab;
				}
			});
			...
			if (last !== undefined && fileItem !== last) {
				...
				let f = (fileItems) => {
					...
					EACH(fileItems, (fileItem) => {
						...
						if (fileItem.checkIsInstanceOf(SkyEditor.Folder) === true) {
							f(fileItem.getItems());
						}
					});
				};
		...
		EVENT('keydown', (e) => {
			...
			// 컨트롤키 누른게 아닌 상태
				...
				if ('abcdefghijklmnopqrstuvwxyz0123456789'.indexOf(e.getKey()) !== -1 && selectedFileItems.length > 0) {
					...
					// 폴더고 열려 있을 때
					if (lastItem.checkIsInstanceOf(SkyEditor.Folder) === true && COUNT_PROPERTIES(lastItem.getItems()) > 0) {
						
						EACH(lastItem.getItems(), (item) => {
							EACH(item.getParent().getChildren(), (item) => {
								if (item.checkIsInstanceOf(SkyEditor.File) === true || item.checkIsInstanceOf(SkyEditor.Folder) === true) {
						...
						if (brotherItem === undefined) {
							...
							if (lastItem.getParent() !== fileTree) {
								...
								REVERSE_EACH(lastItem.getParent().getParent().getChildren(), (child) => {
									if (child.checkIsInstanceOf(SkyEditor.File) === true || child.checkIsInstanceOf(SkyEditor.Folder) === true) {
			...
			if (e.getKey() === 'Control' || e.getKey() === 'Meta') {
				isControlMode = true;
			...
			if (e.getKey() === 'Enter') {
				...
				if (UUI.CONFIRM.getCount() === 0 && selectedFileItems.length > 0) {
					...
					EACH(selectedFileItems, (selectedFileItem) => {
						if (selectedFileItem.checkIsInstanceOf(SkyEditor.Folder) === true) {
							selectedFileItem.open();
						} else if (selectedFileItem.checkIsInstanceOf(SkyEditor.File) === true) {
			...
			if (selectedFileItems.length > 0) {
				...
				if (e.getKey() === 'ArrowUp') {
					...
					if (brotherItem === undefined) {
						...
						if (brotherItem.checkIsInstanceOf(SkyEditor.Folder) === true && COUNT_PROPERTIES(brotherItem.getItems()) > 0) {
					...
					if (document.activeElement.tagName !== 'TEXTAREA' && document.activeElement.tagName !== 'INPUT') {
						...
						else if (lastItem.checkIsInstanceOf(SkyEditor.Folder) === true) {
							lastItem.fireEvent('doubletap');
						}
					}
				...
				if (findText !== '') {
								...
								while (total - folderNames.length < fileNames.length) {
									...
									load({
										...
										if (foundLineInfos.length > 0) {
											...
											fileTree.addItem({
												key : path,
												item : foundFile = SkyEditor.FoundFile({
													path : path,
					...
					EACH(selectedFileItems, (item) => {
						...
						if (item.getFTPInfo() === undefined) {
							...
							if (item.checkIsInstanceOf(SkyEditor.Folder) === true) {
								loadFiles(item.getPath(), (folderNames, fileNames) => {
									search(item.getPath(), folderNames, fileNames)
								});
							}
							
							else if (item.checkIsInstanceOf(SkyEditor.File) === true) {
